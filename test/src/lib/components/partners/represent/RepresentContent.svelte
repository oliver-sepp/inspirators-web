<script lang="ts">
    import arrow from '$lib/assets/arrow-right.png';
    import number01 from '$lib/assets/01.png';
    import ProgressChef from '$lib/assets/represent/represent-image.webp';
</script>

<div class="flex flex-col justify-between md:mt-[15vh] mb-[5vh] md:mb-[10vh]">
    <div class="flex flex-col justify-evenly w-full m-4 mb-20 md:flex-row">
        <div class="w-full text-[#05266A] mx-3 pb-14 relative md:w-2/5 mx-0">
            <h1 class="text-[3rem] md:text-[8vh] text-wrap">
                Progress Software - Chef
            </h1>
            <span class="absolute bottom-0 left-0 w-1/2 border-b-4 border-[#7FC8CB]"></span>
        </div>

        <div class="flex flex-row mt-10 ml-4" >
            <img src={arrow} alt="arrow" class="max-h-10 md:max-h-20 w-auto mr-7">
            <img src={number01} alt="01" class="max-h-10 md:max-h-20 w-auto mr-7">
        </div>
    </div>
    <div class="flex flex-col justify-center gap-[10vw] text-[#05266A] md:flex-row gap-[5vw]">
        
        <div class="flex flex-col space-y-4 mx-10 w-5/6 text-[20px] md:w-1/6 lg:w-2/7">
            <p>
                Chef Enterprise Automation Stack (EAS) is a comprehensive automation platform designed to support DevSecOps teams in building, deploying, managing, and securing applications across any infrastructure. With EAS, teams can establish a shared set of tools and processes to enhance collaboration and alignment. Additionally, compliance testing is integrated into every stage of the technology lifecycle to ensure the highest levels of security and governance. By enabling consistency, velocity, and security in application delivery, EAS empowers teams to deploy applications to any infrastructure with ease and confidence.
            </p>
        </div>

        <div class="w-1/1 flex justify-center items-start md:block md:w-2/6 lg:w-2/7 overflow-hidden">
            <img 
            src={ProgressChef} 
            alt="Pregress Software - Chef"
            class="w-9/10 sm:w-full h-auto object-contain rounded-lg shadow-lg">
        </div>
        
    </div>
</div>