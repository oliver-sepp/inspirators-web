<div class="flex flex-col justify-between md:mt-[15vh] mb-[5vh] md:mb-[10vh]">
    <div class="flex flex-col justify-evenly w-full m-4 mb-20 md:flex-row">
        <div class="w-full text-[#05266A] mx-3 pb-14 relative md:w-2/5 mx-0">
            <h1 class="text-[3rem] md:text-[4rem] lg:text-[3rem]">
                Eesti Gaas
            </h1>
            <span class="absolute bottom-0 left-0 w-1/2 border-b-4 border-[#7FC8CB]"></span>
        </div>

        <div class="flex flex-row mt-10 ml-4" >
            <img src="/arrow-right.png" alt="arrow" class="max-h-10 md:max-h-15 lg:max-h-20 w-auto mr-7">
            <img src="/01.png" alt="01" class="max-h-10 md:max-h-15 lg:max-h-20 w-auto mr-7">
        </div>
    </div>
    <div class="flex flex-col justify-center items-center min-[1000px]:items-start md:gap-[10vw] text-[#05266A] min-[1000px]:flex-row gap-[5vw]">
        
        <div class="flex flex-col space-y-4 lg:mx-10 w-5/6 text-[20px] lg:w-2/7">
            <p>
                Near real-time analytics solution for Eesti Gaas AS (daughter company of listed company Tallink Group).
            </p>
            <p>
                Inspirators! created a data collection and analytic solution that is up and running 24/7, allowing better a base for wiser decisions. The existing manual solution was replaced with automation.
            </p>
            <div class="flex flex-row mb-4 items-start">
                <img src="/checkmark.png" alt="checkmark" class="w-6 object-contain mr-4 pt-1.5">
                <p>
                    <b>
                        The solution is based on #Azure technology
                    </b>
                </p>
            </div>
            <div class="flex flex-row mb-4 items-start">
                <img src="/checkmark.png" alt="checkmark" class="w-6 object-contain mr-4 pt-1.5">
                <p>
                    <b>
                        Relevant data update speed increased 1,000 %
                    </b>
                </p>
            </div>
            <p>
                Together with the Eesti Gaas business team, IT team, and Inspirators!, it took 1 day to design the solution from scratch. In 5 days, Inspirators! launched the first working concept, and then go on to fine-tune the details. New analytics solution was delivered in just a few weeks.
            </p>
        </div>

        <div class="w-1/1 flex justify-center items-start md:block md:w-2/6 lg:w-2/7 overflow-hidden">
            <img 
            src="/energy/energy-1.png" 
            alt=""
            class="w-9/10 sm:w-full h-auto object-contain rounded-lg shadow-lg">
        </div>
        
    </div>
</div>