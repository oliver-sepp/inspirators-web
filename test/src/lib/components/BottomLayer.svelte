<script lang="ts">
import bigA from '$lib/assets/bg-big-A.png';
import bigI from '$lib/assets/bg-big-I.png';
import bigDark from '$lib/assets/bg-big-dark.png';
</script>

<div class="absolute w-full -z-10 overflow-hidden flex flex-col md:flex-row mt-[10vh] md:mt-[35vh] lg:mt-[25vh]">
  
  <div class="lg:w-1/12">

  </div>

  <div class="w-full flex justify-center items-center lg:w-11/12">
    
    <div class="flex w-fit">
      
      <img src={bigA} alt="Big A" class="h-[25vh] sm:h-[30vh] md:h-[40vh] lg:h-[65vh] flex-1 object-contain">
      <img src={bigI} alt="Big I" class="h-[25vh] sm:h-[30vh] md:h-[40vh] lg:h-[65vh] flex-1 object-contain">
    
    </div>
  
  </div>

  <img src={bigDark} alt="Big Dark" class="absolute hidden min-[800px]:inline mt-20 md:mt-0 right-0 h-[35vh] md:h-[40vh] lg:h-[70vh] w-auto object-contain pt-4">

</div>