...
>
  ...
  <div class="hidden md:flex items-center gap-2 lg:gap-15 flex-shrink-0">
    <div class="text-sm lg:text-[19px] flex gap-1 lg:gap-8 items-center">
      {#each links as link}
	  <div 
	  use:clickOutside
	  class="relative inline-block">
		...
		{#if subMenuOpen && active === link}
        	<div
			class="absolute top-full left-1/2 -translate-x-1/2 mt-6 bg-gray-100 shadow-lg rounded-md p-2 min-w-[200px] z-50">
          {#each getSubMenuItems(link as keyof typeof menuData) as item}
            ...
            <a 
              href={url}
              class="block py-2 rounded-md px-4 hover:bg-[#7FC8CD] text-[#05266A] whitespace-nowrap"
			  on:click={(e) => {
				e.preventDefault();
				e.stopPropagation(); 
				goto(url); 
    ...
    <div class="flex items-center gap-2 lg:gap-6 ml-2 lg:ml-0">
      ...
      <div class="relative flex-shrink-0">
        ...
        {#if languagePopUpOpen}
          <div
		   use:clickOutside
            class="absolute top-full right-0 mt-6 w-32 bg-gray-100 shadow-xl rounded-lg border-0 p-2 z-[60]"
          >
            <LanguagePopUp 
  ...
  class="fixed z-[9999] text-center top-16 sm:top-20 left-0 right-0 w-full bg-white text-[#05266A] p-5 md:px-8 overflow-y-auto shadow-md">
    {#each links as link}
      <div class="mb-2">
        ...
        {#if subMenuOpen && active === link}
          <div class="mt-2 bg-gray-100 rounded-md p-2 mx-auto max-w-[300px]">
            {#each getSubMenuItems(link as keyof typeof menuData) as item}
		  	{@const category = link as keyof typeof menuData}
  			{@const url = menuData[category][item as keyof typeof menuData[typeof category]]}
            <a 
